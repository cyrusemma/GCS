                ...
                return (
                  <div
                    ...
                    className={`relative aspect-square rounded-lg flex items-center justify-center text-sm ${
                      ev ? "font-semibold text-crown-blue bg-crown-white" : "text-gray-700"
                    }`}
                  >
                    {day}
                    {ev && (
            ...
            {announcements.map((a) => (
              <motion.div
                ...
                className={`relative rounded-xl p-6 shadow-sm border ${
                  a.urgent ? "bg-crown-gold/10 border-crown-gold/40" : "bg-crown-white border-gray-100"
                }`}
              >
                <Pin className="absolute -top-2 left-5 text-crown-blue rotate-45" size={22} />
                <div className="flex items-center gap-2 mb-2">
                  {a.urgent && <AlertTriangle size={18} className="text-crown-gold" />}
                  <h3 className="font-poppins font-semibold text-crown-blue">{a.title}</h3>
                </div>
          ...
          >
            ...
            >
              <button
                ...
                className="absolute top-4 right-4 h-9 w-9 rounded-full bg-white/90 text-gray-700 flex items-center justify-center shadow"
              >
                <X size={20} />
              </button>
              <img
              ...
              <div className="p-6 sm:p-8">
                <div className="flex items-center gap-3 text-xs text-gray-500 mb-3">
                  ...
                  <span className="flex items-center gap-1">
                    <Calendar size={14} className="text-crown-blue" /> {article.date}
                  </span>
                </div>
                <h2 className="font-poppins font-bold text-2xl text-crown-blue mb-4">{article.title}</h2>
